...
| **Execution time** | 10 seconds per request    | Maximum processing time allowed per single request           |
...
Each running container in your Globe deployment has access to **256MB of memory**. This memory is shared across:

- Your application code
- Libraries, packages, and dependencies
- In-memory caches
- Active connections and stateful data

If your application exceeds this memory limit, Globe may **terminate the container** to maintain platform stability. Any active requests handled by that container will fail as a result.

## CPU Usage

Globe dynamically monitors CPU load across the platform. There's no hard limit per container instead, applications using excessive CPU for extended periods may have requests **terminated** to protect overall system performance.

## Execution Time

Each request to your application is allowed a maximum of **10 seconds** from start to finish, from the time Globe receives the request to when the response is completed.

If a request takes longer than 10 seconds, it will be **automatically terminated**, regardless of its progress.
