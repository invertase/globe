...
- Enables **instant rollbacks** and **A/B testing** deployments
...
Globe automatically routes requests to the **nearest compute region** to minimize latency. However, this may not always provide the best performance.
...
Globe handles region availability intelligently:

- If a single preferred region is specified but unavailable, Globe falls back to standard routing (nearest available region)
- If multiple preferred regions are specified, Globe selects the closest available one
- If all preferred regions are unavailable, standard routing applies
