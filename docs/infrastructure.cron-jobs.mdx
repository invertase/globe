...
Cron jobs can be used to periodically perform a HTTP request to your production Globe deployment. You can specify the cron schedule and the path to request.
...
Whenever a new production deployment is successful, the cron jobs will be sychronized and shown in the dashboard.
...
For example, using [`shelf_router`]() this can be achived as follows:
...
- The ID can only contain lowecase letters, numbers, and underscores (`a-z0-9_`).
