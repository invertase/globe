...
Jaspr is a modern web framework for building websites in Dart. It supports both **client-side** and **server-side** rendering. Globe currently only supports **server-side** rendered Jaspr applications. This page outlines the requirements and deployment behaviour when deploying a Jaspr application to Globe.

<Info>
  Globe currently only supports the deployment of **server-side rendered (SSR)**
  Jaspr applications.
</Info>
