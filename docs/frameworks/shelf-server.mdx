...
<Info>
  In order for Globe to correctly run your backend application, it must be able
  to listen to the `PORT` environment variable, for example, using the
...
- [Learn about managing your deployments](/core-concepts/deployments)
- [Learn about the Globe CLI](/cli)
- [Integrate with GitHub](/core-concepts/github-integration)
