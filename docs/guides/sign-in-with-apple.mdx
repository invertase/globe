...
First, you need to create the necessary identifiers and key in Apple Developer Portal. You'll configure the web authentication URLs after deploying to Globe in Step 7.
...
4. **Create a Services ID**:
   ...
   - Click **Continue**, then **Register**.
   - **Note**: Do not configure Sign in with Apple yet - you'll do this after deploying to Globe.
...
- Add your Apple credentials to this file:
  ...
  **Note**: Replace `your-app` with your actual Globe project name. You'll get this URL after deploying to Globe in Step 6. For now, you can use a placeholder (e.g., `https://placeholder.globeapp.dev/auth/callback`) - you'll update it after deployment.

- **Important**: Add `.env` to your `.gitignore` file.
- **Note**: Your Team ID can be found in the top-right corner of the Apple Developer Portal.
- **Important**: For the `APPLE_PRIVATE_KEY` value:
...
- Next, create a root middleware to make these variables available to all routes. Create a file at `routes/_middleware.dart`:
...
- During deployment, you'll be prompted:
  ...
  ```
  Select **"Upload local environment variables"** to ensure all the environment variables we set earlier (including `APPLE_CLIENT_ID`, `APPLE_TEAM_ID`, `APPLE_KEY_ID`, and `APPLE_PRIVATE_KEY_PATH`) are uploaded to Globe.
...
- **Update your `.env` file** with your actual Globe URL:
  ```env
  APPLE_REDIRECT_URI=https://your-app.globeapp.dev/auth/callback
  ```
...
<Info>
  **Alternative Options for Local Development:** If you prefer to test locally
  before deploying, you can use: - **ngrok**: Run `ngrok http 8080` to get an
  HTTPS URL that tunnels to localhost - **Local domain**: Configure a local
  domain via hosts file with a self-signed certificate However, using your Globe
  production URL is the simplest approach and allows you to test the complete
  flow.
</Info>

## What's Next

- **Get Started Quickly**: Use our [Apple OAuth template](https://globe.dev/templates/dartfrog_apple_auth/) to bootstrap your project with all the code from this guide and more.
