...
- Create `main.dart` at your project root with the following:
  ...
  Future<HttpServer> run(Handler handler, InternetAddress ip, int port) async {
    ...
    // Auto-run pending migrations only during Globe's build phase
...
`Jao.configure()` initializes the database connection. Migrations run only when `GLOBE_BUILD_ENV` is set (during Globe's build phase), so they are not run on every cold start.
