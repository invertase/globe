...
Before creating migrations, register your `Note` model so the Jao CLI knows about it. Open `bin/migrate.dart` and set `modelSchemas` to include `Notes.schema`:
...
Create `main.dart` at your project root with the following to initialize Jao and run migrations on startup:
...
`Jao.configure()` initializes the database connection and `runner.migrate()` applies any pending migrations. This runs once at startup, so there's no need for per-request middleware.
...
Create a file at `routes/notes/index.dart`:
...
Create a file at `routes/notes/[id].dart`:
...
The Dart `sqlite3` package (which Jao uses) bundles its own SQLite binary by default. On Globe, the runtime supplies the database layer, so the app must use that instead. Add this to your `pubspec.yaml` so the package loads SQLite from the runtime rather than bundling it (see the [sqlite3 build hook options](https://pub.dev/documentation/sqlite3/latest/topics/hook-topic.html)):
