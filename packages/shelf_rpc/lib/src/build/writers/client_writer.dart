...
/// The writer for the client RPC code.
...
''');
  ...
  Map<String, List<SourcedRpcProcedure>> _groupProceduresByPath(
    List<SourcedRpcProcedure> procedures,
  ) {
    final routes = <String, List<SourcedRpcProcedure>>{};
  ...
  void _writeNestedRouteGetters(
    SourcedEntrypoint entrypoint,
    Map<String, List<SourcedRpcProcedure>> routes,
  ) {
    final topLevelPaths = routes.keys
  ...
  void _writeProcedureMethod(SourcedRpcProcedure procedure) {
...
extension on SourcedEntrypoint {
  /// The class name of the entrypoint.
