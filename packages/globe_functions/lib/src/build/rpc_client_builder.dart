...
''');
  ...
  $returnType ${func.functionName}(${parameters.join(', ')}) async {
    final response = await client.post(
      ...
      body: jsonEncode({
        'positional': [${positionalArgs.map((arg) => 'serialize(${_getSerializerType(paramTypes[arg]!)}, $arg)').join(', ')}],
        'named': {${namedArgs.map((arg) {
                final name = arg.split(':')[0].substring(1).replaceAll("'", ''); // Remove quotes
                final value = arg.split(':')[1].trim();
                return "'$name': serialize(${_getSerializerType(paramTypes[name]!)}, $value)";
              }).join(', ')}},
      }),
    );
...
''');
  ...
  String _generateResponseHandler(SourcedType returnType) {
    ...
    return 'deserialize(${_getSerializerType(returnType)}, $resultExtraction) as ${returnType.type}';
  }
